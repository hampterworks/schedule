...
const Selector = styled.div<{ x: number; y: number }>`
    position: absolute;
    cursor: pointer;
    top: ${(props) => props.y.toString() + '%'};
    left: ${(props) => props.x.toString() + '%'};
    height: 15px;
    width: 15px;
...
const PositionSelector: React.FC<PositionSelectorProps> = ({backgroundPosition, setBackgroundPosition, ...props}) => {
  const transformPosition =
    backgroundPosition === 'center'
      ? {x: 50, y: 50}
      : {
        x: parseInt(backgroundPosition.split(' ')[0]?.replace('%', '') ?? '50'),
        y: parseInt(backgroundPosition.split(' ')[1]?.replace('%', '') ?? '50')
      }
  ...
  useEffect(() => {
    setBackgroundPosition(`${position.x.toString() + '%'} ${position.y.toString() + '%'}`)
  }, [position])
